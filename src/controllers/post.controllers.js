...
async function createPostController(req, res) {
    ...
    try {
        const post = await postService.createPostService({ title, banner, text },
            userId
        );
...
async function deletePostController(req, res) {
    ...
    try {
        ...
        return res.send({ message: "Postagem excluída com sucesso" });
    } catch (err) {
...
async function commentPostController(req, res) {
    ...
    try {
        ...
        return res.send({
            message: "Comentário concluído com sucesso!",
        });
    } catch (e) {
        return res.status(500).send(e.message);
    }
